...
class User(db.Model):
    ...
    type_id = db.Column(db.Integer, db.ForeignKey('type.id', ondelete='CASCADE'), nullable=False)
    type = db.relationship('Type', backref=db.backref('type', lazy='dynamic'))
    area_id = db.Column(db.Integer, db.ForeignKey('area.id', ondelete='CASCADE'), nullable=False)
    area = db.relationship('Area', backref=db.backref('area', lazy='dynamic'))
